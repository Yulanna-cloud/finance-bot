...
FAMILY_MEMBERS = {
    ...
    "дианы":     "Диана Ш.",
    "диану":     "Диана Ш.",
    # Алексей — полное и краткое Лёша/Леша
    "алексей":   "Алексей П.",
    "алексею":   "Алексей П.",
    "алексея":   "Алексей П.",
    "алёша":     "Алексей П.",
...
CATEGORY_RULES = {
    ...
    "Хозтовары": ["хозтовар", "хоз товар", "посуд", "тарелк", "кастрюл", "сковород",
                     "контейнер", "органайзер", "вешалк", "прищепк", "швабр", "ведро",
                     "совок", "дуршлаг", "половник", "губк", "тряпк", "щётк для",
                     "мусорные пакет", "пакеты для мусора", "фольг", "пищевая плёнк"],
    "Бытовая техника": ["вакуматор", "пылесос", "стиральная машина", "посудомоечн",
                     "холодильник", "микроволновк", "свч печь", "мультиварк",
                     "блендер", "миксер", "тостер", "электрочайник", "соковыжималк",
                     "кофемашин", "кофеварк", "фен", "утюг", "кондиционер",
                     "обогреватель", "увлажнитель", "наушники", "колонка",
                     "телевизор", "ноутбук", "планшет", "монитор", "принтер",
                     "роутер", "повербанк"],
    ...
    "Связь":        ["мтс", "билайн", "мегафон", "теле2", "tele2", "yota", "йота", "симка", "тариф"],
    "Табак":        ["сигарет", "табак", "папирос", "вейп"],
    "Доход":        ["зарплата", "аванс", "оклад", "фриланс", "подработка",
                     "пришло", "приход", "получила", "получил", "прислал", "прислала"],
    "Переводы":     ["перевод", "сбп", "перевела", "перевел"],
...
ITEM_CATEGORY_RULES = [
    ...
    ("Хозтовары",      ["хозтовар", "хоз товар", "посуд", "тарелк", "кастрюл", "сковород",
                        "контейнер", "органайзер", "вешалк", "прищепк", "швабр", "ведро",
                        "совок", "дуршлаг", "половник", "губк", "тряпк"]),
    ("Бытовая техника", ["вакуматор", "пылесос", "стиральная машина", "посудомоечн",
                         "холодильник", "микроволновк", "мультиварк", "блендер",
                         "миксер", "тостер", "электрочайник", "соковыжималк",
                         "кофемашин", "кофеварк", "фен", "утюг", "кондиционер",
                         "обогреватель", "увлажнитель", "наушники", "колонка",
                         "телевизор", "ноутбук", "планшет", "монитор", "принтер",
                         "роутер", "повербанк"]),
...
Животные, Красота, Бытовая химия, Хозтовары, Бытовая техника, Одежда, Развлечения, Подписки, Подписки ИИ, Связь, Табак, Доход, Переводы, Прочее

Особые правила:
- Клод / ЧатГПТ / claude / chatgpt / openai → категория "Подписки ИИ"
- МТС / Билайн / Мегафон / Теле2 → категория "Связь"
- перевод/сбп + описание товара (одежда, еда, обучение и т.д.) → используй категорию ТОВАРА, не "Переводы" (это оплата ИП с карты на карту)
- перевод/сбп без описания товара или просто по имени → категория "Переводы"
...
Правила:
- wildberries/вайлдберриз/wb/вб → магазин "Wildberries"; категорию определяй по названию товара
  (одежда/обувь → "Одежда", бытовая техника/электроника/приборы → "Бытовая техника", и т.д.);
  если товар не назван — категория "Одежда" (это самые частые заказы)
- ozon/озон → магазин "Ozon"; категорию определяй по названию товара так же, как для Wildberries
- танцы/секция/кружок/урок/репетитор → категория "Обучение"
- зарплата/аванс/пришло/приход → тип "доход"
- Клод / ЧатГПТ / claude / chatgpt / openai → категория "Подписки ИИ"
- МТС / Билайн / Мегафон / Теле2 → категория "Связь"
- перевод/сбп + описание товара (одежда, еда, обучение) → категория ТОВАРА (это оплата ИП)
- перевод/сбп без описания товара → категория "Переводы"
- НИКОГДА не ставь "Я", "Вы", "Вас", "мне", "тебе" в поле отправитель или получатель — только реальные имена людей
...
Категории: Продукты, Бытовая химия, Хозтовары, Бытовая техника, Табак, Красота, Одежда, Прочее"""
...
Категории: Продукты, Кафе, Транспорт, Жилье, Коммуналка, Медицина, Одежда, Развлечения, Подписки, Табак, Бытовая химия, Хозтовары, Бытовая техника, Доход, Прочее
Выписка:
{text[:4000]}"""
        response = groq_client.chat.completions.create(
            model="llama-3.3-70b-versatile",
