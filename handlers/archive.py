"""
Обработчик команды /archive — архивирует текущий месяц и очищает таблицу.
Также обрабатывает умные запросы типа "сколько пришло от Алексея П"
"""
import logging
from telegram import Update
from telegram.ext import ContextTypes
from services.sheets_service import archive_month, smart_query

logger = logging.getLogger(__name__)


async def handle_archive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("📦 Архивирую текущий месяц...")
    try:
...
async def handle_smart_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ Не смогла обработать запрос.")
