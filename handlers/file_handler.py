...
# Внутренние переводы между своими счетами — всегда пропускаем
SKIP_INTERNAL = [
    "vklad-karta", "karta-vklad", "sberbank onl@in"
]

# Члены семьи — их пишем в поле "получатель"
FAMILY_MEMBERS = [
    "маргарита п", "маргарита алексеевна",
    "диана ш", "ш. диана", "диана александровна",
    "райса г", "г. райса", "райса махмутовна",
    "юланна г", "салават г", "дамир г", "ольга г",
    "алексей п", "п. алексей", "алексей георгиевич",
]
...
ПРАВИЛО 1 — ОБЯЗАТЕЛЬНО ПРОПУСТИ (не включай в результат):
- Строки с "SBERBANK ONL@IN", "VKLAD-KARTA", "KARTA-VKLAD" — внутренние переброски между счетами
- Строки с PYATEROCHKA, KRASNOE, BELOE, MAGNIT, LENTA, PEREKRESTOK, DIXI, SPAR — продуктовые магазины
- Строки с "Нэсп Без Идентификации" или "Без Идентификации"

ПРАВИЛО 2 — Тип операции:
- Сумма со знаком + = ДОХОД (тип "доход")
- Сумма без знака или с минусом = РАСХОД (тип "расход")
- ATM, банкомат, выдача наличных = тип "наличные"

ПРАВИЛО 3 — Члены семьи (пиши в поле "получатель", поле "магазин" = ""):
Маргарита П. / Маргарита Алексеевна, Диана Ш. / Диана Александровна, Райса Г. / Райса Махмутовна,
Юланна Г., Салават Г., Дамир Г., Ольга Г., Алексей П. / Алексей Георгиевич
- Перевод члену семьи: магазин="", получатель="Имя Фамилия"
- Приход от члена семьи: магазин="", получатель="Имя Фамилия"

ПРАВИЛО 4 — Все остальные люди в переводах (не из списка семьи) — это частники/ИП:
- Пиши их имя в поле "магазин", поле "получатель" = ""
- Пример: "Перевод для В. Динара Фаниловна" → магазин="Динара Ф.", получатель=""

ПРАВИЛО 5 — Магазины и кафе (не переводы людям):
- поле "магазин" = название магазина (убери город и RUS)
- поле "получатель" = ""
- Примеры: "BUTIK LILIYA STERLITAMAK RUS" → магазин="Butik Liliya"
...
ПРАВИЛО 6 — Яндекс:
- "Яндекс" + ПРИХОД (знак +) → категория="Доход", получатель="Алексей П.", тип="доход", магазин=""
- "Яндекс" + РАСХОД → категория="Подписки", магазин="Яндекс", получатель=""

ПРАВИЛО 7 — Категории для переводов семье:
- Маргарита П. или Диана Ш. (расход) → категория="Дети"
- Алексей П. (расход) → категория="Переводы"
- Алексей П. (приход) → категория="Доход"
- Диана Ш. (приход) → категория="Доход"
- Райса Г., Салават Г. и другие родственники → категория="Семья"
- Любой другой приход от конкретного человека → категория="Доход"

Формат каждого объекта:
{{"дата":"DD.MM.YYYY","сумма":число положительное,"тип":"расход/доход/наличные","категория":"...","магазин":"...","описание":"","получатель":"..."}}
...
{full_text[:6000]}"""
        ...
        for item in items:
            ...
            recv = item.get("получатель", "")
            shop = item.get("магазин", "")
